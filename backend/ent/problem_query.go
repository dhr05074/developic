...
func (pq *ProblemQuery) QueryRecords() *RecordQuery {
...
func (pq *ProblemQuery) WithRecords(opts ...func(*RecordQuery)) *ProblemQuery {
	query := (&RecordClient{config: pq.config}).Query()
...
func (pq *ProblemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Problem, error) {
	...
	if query := pq.withRecords; query != nil {
		if err := pq.loadRecords(ctx, query, nodes,
			func(n *Problem) { n.Edges.Records = []*Record{} },
			func(n *Problem, e *Record) { n.Edges.Records = append(n.Edges.Records, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProblemQuery) loadRecords(ctx context.Context, query *RecordQuery, nodes []*Problem, init func(*Problem), assign func(*Problem, *Record)) error {
	...
	query.Where(predicate.Record(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(problem.RecordsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.problem_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "problem_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "problem_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
