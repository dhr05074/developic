...
func (rq *RecordQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Record, error) {
	...
	if query := rq.withProblem; query != nil {
		if err := rq.loadProblem(ctx, query, nodes, nil,
			func(n *Record, e *Problem) { n.Edges.Problem = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RecordQuery) loadProblem(ctx context.Context, query *ProblemQuery, nodes []*Record, init func(*Record), assign func(*Record, *Problem)) error {
	...
	for i := range nodes {
		if nodes[i].problem_records == nil {
			continue
		}
		fk := *nodes[i].problem_records
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "problem_records" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
