...
export const problemIdState = atom<string | null>({
    ...
    effects: [
        ({ setSelf, onSet }) => {
            const savedData = localStorage.getItem(problemIdState.key);
            // setSelf: atom 값을 설정 혹은 재설정
            if (savedData) setSelf(JSON.parse(savedData));
            ...
            onSet((newValue, _, isReset) => {
                isReset
                    ? localStorage.removeItem(problemIdState.key)
                    : localStorage.setItem(problemIdState.key, JSON.stringify(newValue));
            });
        },
    ],
});
